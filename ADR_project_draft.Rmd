...
```{r fig.width=9, fig.height=6, fig.cap="OLS vs QR under high leverage outliers"}
# Plot 1: Clean data
...
```{r fig.width=9, fig.height=6, fig.cap="OLS vs QR under random outliers"}
...
evals <- bind_rows(
  ...
  evaluate_model(y_outliers, y_clean, X1, "Highe Leverage Outliers", method = "qr"),
...
X1 <- data_base$x1
y_base <- data_base$y
contam_levels <- seq(0, 1, by = 0.02)  # 0% to 50% de outliers
...
ggplot(results, aes(x = Contamination, y = Slope, color = Method)) +
  ...
  scale_color_manual(values = c("OLS" = "red", "QR" = "blue")) +
  labs(
...
```{r fig.width=9, fig.height=6, fig.cap="Slope vs Quantile for heteroskedastic data", fig.pos="h"}
